...
def verilog_parser(verilog_file_path, parsed_lib_json_path, verbose=0):
    ...
    if not os.path.exists(verilog_file_path):
        raise FileNotFoundError(f"The file '{verilog_file_path}' does not exist!")

    with open(verilog_file_path, 'r') as v_file:
        file_content = v_file.read().replace('\n', '')  # Remove all newline characters
        if not file_content:
            raise ValueError(f"The file '{verilog_file_path}' is empty!")
    ...
    for line in formatted_lines:
        ...
        # find all the output ports
            ...
            _output, _input = map_pins_to_input_output(parsed_technology_library, gate_type, key_value_pairs)
